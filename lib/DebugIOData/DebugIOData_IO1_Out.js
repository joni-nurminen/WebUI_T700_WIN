require([
	...
	"dojo/text!./lib/DebugIOData/templates/DebugIOData_IO1_Out.html",
	"dgrid/OnDemandGrid",
	"dgrid/Selection", 
	"dgrid/selector", 
	"dojo/store/Memory",
	...
	"dijit/registry",
		...
		function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, template, Grid,Selection,selector,Memory,JsonRest,json,Cache,LangSupport,resources,ready,registry){
			return  declare("DebugIOData_IO1_Out", [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
				
				templateString: template,
				card_1_out : resources.card_1_out,
			...
			//	this.IO1Store = new Memory();
				this.IO1Store = new Cache(new JsonRest({target:"get_debugio_data.php/1_out"}), new Memory());
				...
				tabs.watch("selectedChildWidget", function(name, oval, nval) // page is loaded
					{
						...
						//    this.IO1Store = new Cache(new JsonRest({target:"get_debugio_data.php/1_in"}), new Memory());
							console.log("out1");
							  self.grid_IO1_collection.refresh();
						}
				...
				// Create grid for IO1
					 this.grid_IO1_collection = new (declare([Grid, Selection]))({
						  store: this.IO1Store,
						  columns: columns_IO1,
						  sortable: false,
		...
		Save: function()
		{
			var idlist = [];
				...
				for (var id in this.grid_IO1_collection.selection)
					{

						idlist.push(this.IO1Store.get(id).id);
					}

					var xhrArgs = 
						{
							  url: "save_debug_io_data.php/11",
